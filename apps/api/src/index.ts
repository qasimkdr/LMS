...
import { bootstrapSuperAdminFromEnv } from './services/bootstrapSuperAdmin.js';
...
app.use('/api/exam-attempts', requireAuth, requireTenant, requireModule('EXAMS'), examAttemptRoutes);
...
app.use('/api/advanced-analytics', requireAuth, requireTenant, requireModule('ANALYTICS'), advancedAnalyticsRoutes);
app.use('/api/fees', sensitiveLimiter, requireAuth, requireTenant, requireModule('FINANCE'), feeRoutes);
app.use('/api/fee-ledger', requireAuth, requireTenant, requireModule('FINANCE'), feeLedgerRoutes);
app.use('/api/fee-adjustments', sensitiveLimiter, requireAuth, requireTenant, requireModule('FINANCE'), feeAdjustmentRoutes);
app.use('/api/finance-settings', sensitiveLimiter, requireAuth, requireTenant, requireModule('FINANCE'), financeSettingsRoutes);
app.use('/api/timetable', requireAuth, requireTenant, requireModule('TIMETABLE'), timetableRoutes);
app.use('/api/school-life', requireAuth, requireTenant, requireModule('LEAVE'), leaveCalendarRoutes);
app.use('/api/storage', sensitiveLimiter, requireAuth, requireTenant, requireModule('STORAGE'), storageRoutes);
app.use('/api/support', requireAuth, requireTenant, requireModule('SUPPORT'), supportRoutes);

app.use((err: any, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
  if (err?.status === 413 || err?.type === 'entity.too.large') return res.status(413).json({ message: 'Request body is too large.' });
  if (err instanceof SyntaxError && 'body' in err) return res.status(400).json({ message: 'Request body contains invalid JSON.' });
  console.error(err);
