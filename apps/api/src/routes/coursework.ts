...
router.put('/assignments/:id/submission', requireRoles('STUDENT'), async(req,res)=>{
 const p=submitSchema.safeParse(req.body); if(!p.success)return res.status(400).json({message:'Invalid submission'}); const schoolId=req.auth!.schoolId!; const profile=await prisma.studentProfile.findFirst({where:{schoolId,userId:req.auth!.userId}}); const assignment=await prisma.assignment.findFirst({where:{id:req.params.id,schoolId,classId:profile?.classId??'none'}}); if(!assignment)return res.status(404).json({message:'Assignment not found'});
