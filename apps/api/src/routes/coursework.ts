...
router.post('/assignments', requireRoles('PRINCIPAL','TEACHER'), async(req,res)=>{
 ...
 if(students.length) await prisma.notification.createMany({data:students.map(s=>({schoolId,userId:s.userId,type:'ASSIGNMENT',title:'New assignment',message:d.title,link:'/student/coursework'}))});
