...
const fileReference = z
  .string()
  .refine(
    (v) => /^https?:\/\//i.test(v) || /^storage:\/\/[0-9a-f-]+$/i.test(v),
  ...
  if (role === "TEACHER")
    return Boolean(
      await prisma.teacherAssignment.findFirst({
        where: { schoolId, teacherId: userId, classId, subjectId },
      }),
    );
  return false;
}
...
const assignmentSchema = z.object({
  ...
  attachmentUrl: fileReference.optional(),
...
router.get(
  ...
  async (req, res) => {
    ...
    if (req.auth!.role === "PARENT")
      return res
        .status(400)
        .json({ message: "Use parent child coursework endpoint" });
    if (req.auth!.role === "TEACHER") {
      const assigned = await prisma.teacherAssignment.findMany({
        where: { schoolId, teacherId: req.auth!.userId },
        select: { classId: true, subjectId: true },
      });
      const OR = assigned.map((a) => ({
        classId: a.classId,
...
router.post(
  ...
  async (req, res) => {
    ...
    if (!p.success)
      return res
        .status(400)
        .json({ message: "Invalid assignment", issues: p.error.flatten() });
    const d = p.data,
    ...
    if (
      ...
      return res.status(403).json({
        message:
          "Class or subject is outside your school, or you are not assigned to it",
      });
    const row = await prisma.assignment.create({
      data: { schoolId, createdById: req.auth!.userId, ...d },
    });
    const students = await prisma.studentProfile.findMany({
      where: { schoolId, classId: d.classId },
      select: { userId: true },
    });
    if (students.length)
...
router.patch(
  ...
  async (req, res) => {
    ...
    if (req.auth!.role === "TEACHER" && item.createdById !== req.auth!.userId)
      return res
        .status(403)
        .json({ message: "Teachers can edit only their own assignments" });
    if (
      (p.data.classId || p.data.subjectId) &&
      !(await canManage(
        req.auth!.userId,
        req.auth!.role,
        schoolId,
        p.data.classId ?? item.classId,
        p.data.subjectId ?? item.subjectId,
      ))
    )
      return res
        .status(403)
        .json({ message: "Not allowed for that class and subject" });
    const row = await prisma.assignment.update({ where: { id }, data: p.data });
    await prisma.auditLog.create({
      data: {
        schoolId,
        actorId: req.auth!.userId,
        ...
        entityType: "Assignment",
        entityId: id,
        beforeData: { title: item.title, dueAt: item.dueAt },
        afterData: p.data,
      },
    });
    res.json(row);
...
router.delete(
  ...
  async (req, res) => {
    const schoolId = req.auth!.schoolId!,
      id = routeParam(req.params.id),
      item = await prisma.assignment.findFirst({
        where: { id, schoolId },
        include: { _count: { select: { submissions: true } } },
      });
    if (!item) return res.status(404).json({ message: "Assignment not found" });
    if (req.auth!.role === "TEACHER" && item.createdById !== req.auth!.userId)
      return res
        .status(403)
        .json({ message: "Teachers can delete only their own assignments" });
    if (item._count.submissions)
      return res
        .status(409)
        .json({ message: "Assignment has submissions and cannot be deleted" });
...
router.put(
  ...
  async (req, res) => {
    ...
    const assignment = await prisma.assignment.findFirst({
      where: { id, schoolId, classId: profile?.classId ?? "none" },
    });
    if (!assignment)
    ...
    const row = await prisma.assignmentSubmission.upsert({
      ...
      create: {
        assignmentId: assignment.id,
        studentUserId: req.auth!.userId,
        textAnswer: p.data.textAnswer,
        attachmentUrl: p.data.attachmentUrl,
        status,
        submittedAt: p.data.submit ? now : null,
      },
      update: {
        textAnswer: p.data.textAnswer,
        attachmentUrl: p.data.attachmentUrl,
...
router.patch(
  ...
  requireRoles("PRINCIPAL", "TEACHER"),
  async (req, res) => {
    const p = grade.safeParse(req.body);
    ...
    if (!sub || sub.assignment.schoolId !== req.auth!.schoolId)
    ...
    if (
      ...
      p.data.score > Number(sub.assignment.maxMarks)
    )
      return res.status(400).json({ message: "Score exceeds maximum marks" });
    const row = await prisma.assignmentSubmission.update({
      where: { id: sub.id },
      data: {
        score: p.data.score,
        feedback: p.data.feedback,
        status: "GRADED",
        gradedAt: new Date(),
      },
    });
...
router.get(
  "/materials",
  requireRoles("PRINCIPAL", "TEACHER", "STUDENT"),
  async (req, res) => {
    const schoolId = req.auth!.schoolId!;
    if (req.auth!.role === "STUDENT") {
      ...
      if (!p?.classId) return res.json([]);
      return res.json(
        await prisma.courseMaterial.findMany({
          where: { schoolId, classId: p.classId },
...
router.patch(
  ...
  async (req, res) => {
    const p = materialSchema.partial().safeParse(req.body),
      ...
      id = routeParam(req.params.id);
    if (!p.success)
      return res.status(400).json({ message: "Invalid material update" });
    ...
    if (req.auth!.role === "TEACHER" && item.createdById !== req.auth!.userId)
      return res
        .status(403)
        .json({ message: "Teachers can edit only their own materials" });
    res.json(
      await prisma.courseMaterial.update({ where: { id }, data: p.data }),
    );
  },
);
router.delete(
  ...
  async (req, res) => {
    const schoolId = req.auth!.schoolId!,
      id = routeParam(req.params.id),
      item = await prisma.courseMaterial.findFirst({ where: { id, schoolId } });
    if (!item) return res.status(404).json({ message: "Material not found" });
    if (req.auth!.role === "TEACHER" && item.createdById !== req.auth!.userId)
      return res
        .status(403)
        .json({ message: "Teachers can delete only their own materials" });
...
router.get(
  ...
  async (req, res) => {
    ...
    if (req.auth!.role === "STUDENT") {
      ...
      if (!p?.classId) return res.json([]);
      return res.json(
        await prisma.syllabusItem.findMany({
          where: { schoolId, classId: p.classId },
          include: { subject: true },
...
router.post(
  ...
  requireRoles("PRINCIPAL", "TEACHER"),
  async (req, res) => {
    const p = syllabusSchema.safeParse(req.body);
    if (!p.success)
      return res.status(400).json({ message: "Invalid syllabus item" });
    const d = p.data,
      schoolId = req.auth!.schoolId!;
    if (
      !(await canManage(
...
router.patch(
  ...
  async (req, res) => {
    ...
    const item = await prisma.syllabusItem.findFirst({
      where: { id, schoolId: req.auth!.schoolId! },
    });
...
router.patch(
  ...
  requireRoles("PRINCIPAL", "TEACHER"),
  async (req, res) => {
    const p = syllabusSchema.partial().safeParse(req.body),
      schoolId = req.auth!.schoolId!,
      id = routeParam(req.params.id);
    if (!p.success)
      return res.status(400).json({ message: "Invalid syllabus update" });
    const item = await prisma.syllabusItem.findFirst({
      where: { id, schoolId },
    });
    if (!item) return res.status(404).json({ message: "Item not found" });
    if (req.auth!.role === "TEACHER" && item.createdById !== req.auth!.userId)
      return res
        .status(403)
        .json({ message: "Teachers can edit only their own syllabus items" });
    res.json(await prisma.syllabusItem.update({ where: { id }, data: p.data }));
  },
);
router.delete(
  "/syllabus/:id",
  ...
  async (req, res) => {
    const schoolId = req.auth!.schoolId!,
      id = routeParam(req.params.id),
      item = await prisma.syllabusItem.findFirst({ where: { id, schoolId } });
    if (!item) return res.status(404).json({ message: "Item not found" });
    if (req.auth!.role === "TEACHER" && item.createdById !== req.auth!.userId)
      return res
        .status(403)
        .json({ message: "Teachers can delete only their own syllabus items" });
    await prisma.syllabusItem.delete({ where: { id } });
    await prisma.auditLog.create({
      data: {
        schoolId,
        actorId: req.auth!.userId,
        action: "SYLLABUS_ITEM_DELETED",
        entityType: "SyllabusItem",
        entityId: id,
        beforeData: { title: item.title },
      },
    });
