...
router.get('/student', requireRoles('STUDENT'), async (req, res) => {
  ...
  const [attendance, attempts, announcements] = await Promise.all([
    prisma.attendanceRecord.findMany({
      where: { studentProfileId: profile.id, attendance: { schoolId } },
      select: { status: true, attendance: { select: { date: true } } },
      orderBy: { attendance: { date: 'desc' } },
    }),
    prisma.examAttempt.findMany({
      where: { schoolId, studentUserId: req.auth!.userId },
      include: { exam: { select: { title: true, totalMarks: true, showResults: true, subject: { select: { name: true } } } } },
    ...
    prisma.announcement.findMany({
      where: {
        ...
        OR: [{ classId: null }, { classId: profile.classId ?? '__none__' }],
        AND: [
...
router.get('/parent/children/:studentId', requireRoles('PARENT'), async (req, res) => {
  ...
  const [attendance, attempts, announcements] = await Promise.all([
    prisma.attendanceRecord.findMany({
      where: { studentProfileId: link.student.id, attendance: { schoolId } },
      select: { status: true, attendance: { select: { date: true } } },
    }),
    prisma.examAttempt.findMany({
      where: { schoolId, studentUserId: link.student.userId, status: 'GRADED' },
      include: { exam: { select: { title: true, totalMarks: true, showResults: true, subject: { select: { name: true } } } } },
    ...
    prisma.announcement.findMany({
      where: {
        schoolId,
        audience: { has: 'PARENT' },
        OR: [{ classId: null }, { classId: link.student.classId ?? '__none__' }],
        AND: [
          { OR: [{ publishAt: null }, { publishAt: { lte: new Date() } }] },
          { OR: [{ expiresAt: null }, { expiresAt: { gte: new Date() } }] },
        ],
