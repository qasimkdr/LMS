...
router.get('/overview', async (req, res) => {
  ...
  const [school, users, students, classes, subjects] = await Promise.all([
    prisma.school.findUnique({ where: { id: schoolId }, select: { id: true, name: true, slug: true, logoUrl: true, description: true, phone: true, email: true, address: true, timezone: true, status: true } }),
...
router.post('/classes', async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ message: 'Invalid class data' });
...
router.post('/subjects', async (req, res) => {
  ...
  if (!parsed.success) return res.status(400).json({ message: 'Invalid subject data' });
  const row = await prisma.subject.create({ data: { schoolId, ...parsed.data } });
  res.status(201).json(row);
});

const studentSchema = z.object({
  firstName: z.string().min(2), lastName: z.string().min(1), email: z.string().email(), username: z.string().min(3), password: z.string().min(8),
  admissionNo: z.string().min(1), classId: z.string().uuid().optional(), section: z.string().optional(), guardianPhone: z.string().optional(),
});
router.post('/students', async (req, res) => {
  const schoolId = req.auth!.schoolId!;
  const parsed = studentSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ message: 'Invalid student data', issues: parsed.error.flatten() });
  const { admissionNo, classId, section, guardianPhone, password, ...userData } = parsed.data;
  const passwordHash = await bcrypt.hash(password, 12);
  const student = await prisma.$transaction(async (tx) => {
    const user = await tx.user.create({ data: { schoolId, role: 'STUDENT', ...userData, email: userData.email.toLowerCase(), passwordHash } });
    const profile = await tx.studentProfile.create({ data: { schoolId, userId: user.id, admissionNo, classId, section, guardianPhone } });
    await tx.auditLog.create({ data: { schoolId, actorId: req.auth!.userId, action: 'STUDENT_CREATED', entityType: 'StudentProfile', entityId: profile.id, afterData: { admissionNo, classId: classId ?? null } } });
    return profile;
  });
  res.status(201).json(student);
});

const brandingSchema = z.object({ name: z.string().min(2).optional(), logoUrl: z.string().url().nullable().optional(), description: z.string().max(1000).nullable().optional(), phone: z.string().nullable().optional(), email: z.string().email().nullable().optional(), address: z.string().max(500).nullable().optional(), timezone: z.string().min(2).optional() });
router.patch('/school', async (req, res) => {
  ...
  await prisma.auditLog.create({ data: { schoolId, actorId: req.auth!.userId, action: 'SCHOOL_PROFILE_UPDATED', entityType: 'School', entityId: schoolId, beforeData: before ? { name: before.name, logoUrl: before.logoUrl } : undefined, afterData: parsed.data } });
  res.json(school);
});
