...
function normalize(value: unknown) {
  ...
  if (Array.isArray(value)) return [...value].map(String).sort().join('|').toLowerCase();
...
router.get('/available', requireRoles('STUDENT'), async (req, res) => {
  ...
  const exams = await prisma.exam.findMany({
    where: { schoolId, classId: profile.classId, AND: [{ OR: [{ startsAt: null }, { startsAt: { lte: now } }] }, { OR: [{ endsAt: null }, { endsAt: { gte: now } }] }] },
    orderBy: { startsAt: 'asc' },
    include: { subject: true, _count: { select: { questions: true } }, attempts: { where: { studentUserId: req.auth!.userId }, select: { id: true, status: true, score: true, percentage: true, passed: true } } },
  });
  res.json(exams);
});
...
router.post('/:attemptId/submit', requireRoles('STUDENT'), async (req, res) => {
  ...
  const percentage = Number(attempt.exam.totalMarks) > 0 ? (score / Number(attempt.exam.totalMarks)) * 100 : 0;
  const passed = attempt.exam.passingMarks != null ? score >= Number(attempt.exam.passingMarks) : null;
  const status = manualNeeded ? 'PENDING_REVIEW' : 'GRADED';
  const updated = await prisma.examAttempt.update({ where: { id: attempt.id }, data: { status, submittedAt: new Date(), score, percentage, passed, gradedAt: manualNeeded ? null : new Date() } });
  res.json(updated);
});
...
router.patch('/:attemptId/grade', requireRoles('TEACHER','PRINCIPAL'), async (req, res) => {
  ...
  for (const grade of parsed.data.answers) {
    ...
    if (grade.score > Number(answer.question.marks)) return res.status(400).json({ message: 'Score exceeds question marks' });
  }
  const updated = await prisma.$transaction(async tx => {
    for (const grade of parsed.data.answers) await tx.studentAnswer.update({ where: { id: grade.answerId }, data: { manualScore: grade.score, feedback: grade.feedback } });
    const fresh = await tx.studentAnswer.findMany({ where: { attemptId: attempt.id } });
    const score = fresh.reduce((sum, a) => sum + Number(a.manualScore ?? a.autoScore ?? 0), 0);
    const percentage = Number(attempt.exam.totalMarks) > 0 ? (score / Number(attempt.exam.totalMarks)) * 100 : 0;
    const passed = attempt.exam.passingMarks != null ? score >= Number(attempt.exam.passingMarks) : null;
    const row = await tx.examAttempt.update({ where: { id: attempt.id }, data: { status: 'GRADED', score, percentage, passed, teacherFeedback: parsed.data.teacherFeedback, gradedAt: new Date() } });
    await tx.auditLog.create({ data: { schoolId, actorId: req.auth!.userId, action: 'EXAM_ATTEMPT_GRADED', entityType: 'ExamAttempt', entityId: attempt.id, afterData: { score, percentage } } });
    return row;
  });
  res.json(updated);
});
...
router.get('/:attemptId/result', requireRoles('STUDENT','PARENT','TEACHER','PRINCIPAL'), async (req, res) => {
  ...
  if (attempt.status !== 'GRADED' && ['STUDENT','PARENT'].includes(req.auth!.role)) return res.status(403).json({ message: 'Result is not ready yet' });
  res.json(attempt);
});
