...
export default function SchoolCard({ school, busy, onStatus }: { school: SchoolSummary; busy?: boolean; onStatus: (status: SchoolSummary['status']) => void }) {
  ...
  return (
    <article className="group relative overflow-hidden rounded-[28px] border border-white/70 bg-white/78 p-5 shadow-[0_18px_55px_rgba(38,82,148,.12)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:shadow-[0_25px_70px_rgba(38,82,148,.2)]">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-sm text-slate-500">{principal ? `${principal.firstName} ${principal.lastName} · ${principal.email}` : 'Principal not assigned'}</p>
          </div>
        </div>
        <IconButton size="small" onClick={(e) => setAnchor(e.currentTarget)} disabled={busy}><MoreVertIcon /></IconButton>
      </div>
      ...
      <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
        ...
        <span className="text-xs font-semibold text-slate-500">{school.studentLimit.toLocaleString()} student limit</span>
      </div>

      <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={() => setAnchor(null)}>
        {(['ACTIVE','GRACE_PERIOD','READ_ONLY','SUSPENDED','CANCELLED'] as const).map((status) => (
          <MenuItem key={status} disabled={status === school.status || busy} onClick={() => { setAnchor(null); onStatus(status); }}>
