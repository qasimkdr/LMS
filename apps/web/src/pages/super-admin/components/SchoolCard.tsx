  ...
  return (
    <article className="group relative overflow-hidden rounded-[28px] border border-white/70 bg-white/78 p-5 shadow-[0_18px_55px_rgba(38,82,148,.12)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:shadow-[0_25px_70px_rgba(38,82,148,.2)]">
        ...
        <div className="flex min-w-0 gap-3">
          <div className="grid size-12 shrink-0 place-items-center overflow-hidden rounded-2xl bg-gradient-to-br from-blue-100 to-indigo-100 shadow-inner">
            {school.logoUrl ? (
              ...
              <BusinessIcon className="text-blue-700" />
            )}
          </div>
          <div className="min-w-0">
            <h3 className="truncate text-lg font-black text-slate-900">{school.name}</h3>
            <p className="truncate text-sm text-slate-500">
              {principal ? `${principal.firstName} ${principal.lastName} · ${principal.email}` : 'Principal not assigned'}
            </p>
          </div>
        </div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-2">
        <Metric icon={<PeopleAltIcon fontSize="small" />} value={school._count.users} label="Users" />
        <Metric icon={<SchoolIcon fontSize="small" />} value={school._count.classes} label="Classes" />
        <Metric icon={<BusinessIcon fontSize="small" />} value={school._count.exams} label="Exams" />
      </div>
      ...
      <div className="mt-4 rounded-2xl bg-slate-50 p-3">
        <div className="flex items-center justify-between gap-2 text-xs">
          <span className="flex items-center gap-1 font-bold text-slate-600">
            ...
            {used} / {school.storageLimitMb} MB
          </span>
          <span className={`font-black ${pct >= 90 ? 'text-rose-600' : pct >= 75 ? 'text-amber-600' : 'text-emerald-600'}`}>
            {pct}%
          </span>
        </div>
        ...
        {school.subscriptionEnd && (
          <p className="mt-2 text-[11px] font-semibold text-slate-500">
            Subscription ends {new Date(school.subscriptionEnd).toLocaleDateString()}
          </p>
        )}
      </div>

      <div className="mt-4 flex items-center justify-between gap-3 border-t border-slate-100 pt-4">
        <Chip size="small" label={school.status.replaceAll('_', ' ')} className="font-bold" />
        <span className="text-xs font-semibold text-slate-500">
          {school.studentLimit.toLocaleString()} students · {school.teacherLimit.toLocaleString()} teachers
        </span>
      </div>

      <Menu anchorEl={anchor} open={Boolean(anchor)} onClose={() => setAnchor(null)}>
        <MenuItem
          disabled={!principal || school.status === 'CANCELLED' || busy}
          onClick={() => {
            setAnchor(null);
            onImpersonate();
          }}
        >
          <SupportAgentRoundedIcon fontSize="small" sx={{ mr: 1 }} />
          Support as Principal
        </MenuItem>
        <MenuItem
          onClick={() => {
            setAnchor(null);
        ...
        >
          <EventRepeatRoundedIcon fontSize="small" sx={{ mr: 1 }} />
          Manage subscription
        </MenuItem>
        <MenuItem
          onClick={() => {
...
function Metric({ icon, value, label }: { icon: ReactNode; value: number; label: string }) {
  return (
    <div className="rounded-2xl bg-slate-50/90 px-3 py-3 text-center transition group-hover:bg-white">
